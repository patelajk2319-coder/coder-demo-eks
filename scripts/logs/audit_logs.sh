#!/usr/bin/env bash
# Export Coder AI governance audit logs to data/audit-logs.json.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "${SCRIPT_DIR}/../.." && pwd)"
OUTPUT_FILE="${ROOT_DIR}/data/audit-logs.json"
...
section "Fetching audit log..."
curl -sf \
  -H "Coder-Session-Token: ${TOKEN}" \
  "${LOCAL_CODER_URL}/api/v2/audit?limit=100&offset=0" \
  | jq '.' > "${OUTPUT_FILE}"

COUNT=$(jq '.count' "${OUTPUT_FILE}" 2>/dev/null || echo 0)
info "Exported ${COUNT} audit events to ${OUTPUT_FILE}"
