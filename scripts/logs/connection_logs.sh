#!/usr/bin/env bash
# Export Coder connection logs to data/connection-logs.json.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "${SCRIPT_DIR}/../.." && pwd)"
OUTPUT_FILE="${ROOT_DIR}/data/connection-logs.json"
...
section "Fetching connection log..."
curl -sf \
  -H "Coder-Session-Token: ${TOKEN}" \
  "${LOCAL_CODER_URL}/api/v2/connectionlog?limit=100&offset=0" \
  | jq '.' > "${OUTPUT_FILE}"

COUNT=$(jq '.count' "${OUTPUT_FILE}" 2>/dev/null || echo 0)
info "Exported ${COUNT} connection events to ${OUTPUT_FILE}"
